...
const union = (auxU1,auxU2) => {
    ...
    for(let dvj=0; dvj<largo2*numAlf; dvj++){
        newRec.push(`q${Number.parseInt(auxU2.g[dvj].charAt(1))+(largo+1)}`);
    }
...
const concatenacion = (aux1,aux2) => {
    ...
    for(let dv=0; dv<largoi*numAlf; dv++){
        newiQ.push(`q${Math.trunc(dv/numAlf)}`);
    }
    for(let dvp=0; dvp<largoi2*numAlf; dvp++){
        newiQ.push(`q${Math.trunc(dvp/numAlf)+largoi}`);
    }

    for(let dvm=0; dvm<largoi*numAlf; dvm++){
        newiTrans.push(aux1.s[dvm]);
    }
    for(let dvi=0; dvi<largoi2*numAlf; dvi++){
        newiTrans.push(aux2.s[dvi]);
    }

    for(let dvo=0; dvo<largoi*numAlf; dvo++){
        newiRec.push(`q${Number.parseInt(aux1.g[dvo].charAt(1))}`);
    }
    for(let dvj=0; dvj<largoi2*numAlf; dvj++){
        newiRec.push(`q${Number.parseInt(aux2.g[dvj].charAt(1))+largoi}`);
    }
    ...
    for(let dvg=0; dvg<(largoi+largoi2); dvg++){
        newiQFinale.push(`q${dvg}`);
    }
    ...
    for(let afg=0;afg<largoi;afg++){
        if(aux1.f[afg]==true){
            newiQ.push(`q${afg}`)
            newiTrans.push('E');
            newiRec.push(`q${Number.parseInt(aux2.i)+largoi-1}`);
        }
    }
